...
def add_mods(new_mod):
    with open('/bot/mods.txt', 'a') as plik:
        plik.write(new_mod + '\n')

@bot.command(name='fast_update')
async def fast_update(ctx, imgur):
    with open('/bot/mods.txt', 'r') as f:
        nicki = [nick.strip().lower() for nick in f.readlines()]
    if ctx.author.name.lower() in nicki:
        with open("wynik.txt", "w") as f:
            f.write(f'{imgur}')
        await ctx.send(f'Wyniki losowania zaktualizowane. Nowe wyniki: {imgur}')
    else:
        await ctx.send('Nie masz uprawnień do aktualizacji wyników.')
